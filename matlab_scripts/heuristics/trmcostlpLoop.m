function cost = trmcostlpLoop(trmodel, trmAnglesIndices, trmAnglesValues, p)
...
cost = trmcostlpLoopPrivate(trmodel, p);

end

function cost = trmcostlpLoopPrivate(trmodel, p)
%TRMCOST_PRIVATE Calculates cost of the transformation.
...
l = zeros(length(coordscell{1,1}),1);

for i = 1:(nModels-1)
    l = l + sqrt(sum((coordscell{i+1} - coordscell{i}).^2,2));
end

cost = sum(l.^p .* trmodel.m);
