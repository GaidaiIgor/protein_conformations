function cost = trmcostpLoop(trmodel, trmAnglesIndices, trmAnglesValues, p)
%TRMCOSTLPLOOP Calculates loop transformation model cost as a function of psi
%   and phi angles if psi and phi are specified.
%   Or just calculates loop transformation model cost if psi and phi
%   are not specified.
...
cost = trmcostpLoopPrivate(trmodel, p);

end

function cost = trmcostpLoopPrivate(trmodel, p)
